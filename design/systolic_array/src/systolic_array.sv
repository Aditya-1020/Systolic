module systolic_array #(
    ...
    logic signed [N-1:0][DATA_WIDTH-1:0] skewed_data, skewed_weight;
    ...
    generate
        for (r = 0; r < N; r++) begin : gen_data_in
            assign data_w[r][0] = skewed_data[r];
        end
        for (c = 0; c < N; c++) begin : gen_weight_in
            assign weight_w[0][c] = skewed_weight[c];
        end
    endgenerate
    ...
    logic _unused_edge;
    always_comb begin
        _unused_edge = &weight_w[N];
        for (int rr = 0; rr < N; rr++)
            _unused_edge = _unused_edge & &data_w[rr][N];
    end
